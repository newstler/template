...
class PrepareRubyLlmV2Upgrade < ActiveRecord::Migration[8.2]
  ...
  def create_usages
    unless table_exists?(:ruby_llm_usages)
      # Template patch: string PKs need the uuid7() default (the generator emits a bare :string id).
      create_table :ruby_llm_usages, id: :string, default: -> { "uuid7()" } do |table|
  ...
  def create_batches
    unless table_exists?(:ruby_llm_batches)
      # Template patch: string PKs need the uuid7() default.
      create_table :ruby_llm_batches, id: :string, default: -> { "uuid7()" } do |table|
